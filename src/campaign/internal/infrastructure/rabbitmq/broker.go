...
// Ce package ne contient que les implementations concretes qui la satisfont.
...
// NewNoopBroker retourne un NoopBroker.
...
func (b *NoopBroker) Publish(_ context.Context, routingKey string, body []byte) error {
	log.Printf("[campaign:broker:noop] publish routingKey=%s body_len=%d", routingKey, len(body))
	return nil
}
...
func (b *NoopBroker) Consume(ctx context.Context, queueName string, _ func([]byte) error) error {
	log.Printf("[campaign:broker:noop] consume queueName=%s (no-op, waiting for context cancellation)", queueName)
	<-ctx.Done()
	return nil
}
