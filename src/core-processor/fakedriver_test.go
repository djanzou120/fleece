...
// nouvelle dependance), permettant d'executer reellement (sans Postgres ni
// RabbitMQ, environnement offline) le SQL emis par on_message_delivered.go et
// on_message_failed.go, y COMPRIS la transaction de remboursement (Begin/
// Exec/Query/Commit/Rollback) — contrairement au harnais equivalent de
// src/api/qa_m019_dbpath_test.go, dont le fakeConn ne supporte pas les
// transactions ("Begin non supporte"). Ce fichier etend le pattern pour
// couvrir Begin/QueryContext/ExecContext au sein d'une transaction, ce qui est
// le coeur de handleMessageFailed (D-M20/D-M12 : reutiliser gosql.NewFromSQLX,
// deja expose par src/go/sql/db.go pour les tests/infrastructure).
//
// Limites assumees (memes que src/api/qa_m019_dbpath_test.go) : ce faux driver
// valide la FORME du SQL emis (table/colonnes/placeholders/ordre des
// arguments) et le branchement du code (0/1 ligne RETURNING, erreur
// technique) — pas la conformite exacte au moteur Postgres reel (deja
// verifiee par revue croisee avec les migrations 0002/0018/0019 dans le
...
// pour simuler le resultat d'un UPDATE ... RETURNING (SELECT via tx.Get/
// s.DB.Get).
...
// l'ordre d'appel). Utilise pour simuler une sequence multi-requetes au sein
// d'une transaction (ex. handleMessageFailed : UPDATE messages RETURNING,
// puis UPDATE wallets RETURNING).
...
type fakeConn struct {
	...
	// queries/args capture l'historique complet (une entree par appel), dans
	// l'ordre chronologique, quel que soit Exec ou Query — pratique pour
	// asserter la sequence exacte de requetes emises par une transaction.
	queries []string
...
// (comportement standard de database/sql : la connexion reservee pour la tx
// est reutilisee directement, le driver.Tx retourne ne sert qu'a Commit/
// Rollback).
...
// lastQuery/lastArgs — la derniere requete executee (Exec OU Query), pratique
// pour les tests qui ne s'interessent qu'a un seul appel (ex. on_message_delivered.go).
...
// appels (aucune vraie semantique transactionnelle a simuler ici — le faux
// driver n'a qu'une seule "connexion" logique, cf. Begin ci-dessus).
...
// sur le faux driver ci-dessus (via gosql.NewFromSQLX, constructeur de test/
// infrastructure expose par src/go/sql/db.go — aucune ecriture dans un champ
// non exporte n'est necessaire).
func newFakeGosqlDB(t *testing.T, conn *fakeConn) *gosql.DB {
	t.Helper()

	name := "flcoreprocmock" + strconv.FormatInt(atomic.AddInt64(&fakeDriverSeq, 1), 10)
...
// valeurs fournies (0, 1 ou plusieurs lignes simulees). Pratique pour les cas
// a une seule colonne RETURNING (ex. balance).
