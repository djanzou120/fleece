...
> **Créé le :** 2026-07-14
> **Statut global :** ✅ Phase 0 Done/PASS · ✅ Phase 1 Done/PASS — Phase 2 (endpoints, M-012..M-019) débloquée
...
| D-M07 | Composition root `src/api/cmd/api/main.go` : `goapp.Context()` créé mais `ctx` non consommé par `http.ListenAndServe` → pas d'arrêt gracieux. Migrer vers `http.Server` + `srv.Shutdown(ctx)`. Uniformiser le logging (mix `golog.Init` + `log.Printf` stdlib pour les erreurs fatales). | Phase 2 (M-012+) |
| D-M08 | `providers/telegram.go` : `MapTelegramResponseStatus` (fonction pure testée) et les constantes de capabilities sont définies mais non encore appelées par `Send()` (stub Phase 1). À câbler dans `Send` en Phase 2. | M-012/Phase 2 |
